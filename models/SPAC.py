...
class Encoder(nn.Module):
    def __init__(self, latent_dim, num_coarse):
        ...
        self.sa_module_2 = PointNet_SA_Module_KNN(num_coarse, 16, 128, [128, 256], group_all=False, if_bn=False)
...
class SPAC(nn.Module):
    def __init__(self, config):
        super(SPAC, self).__init__()
        self.num_dense = config.num_dense
        self.num_coarse = config.num_coarse
        self.encoder = Encoder(config.latent_dim, self.num_coarse)
    ...
    def forward(self, partial, junction_index):
        ...
        if not dense == None:
            dense = torch.cat([dense, partial], dim=1).contiguous()
