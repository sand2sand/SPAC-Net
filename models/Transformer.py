...
import numpy as np
from extensions.KNN.KNN import KNN
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, q, v, self_knn_index=None, cross_knn_index=None):
        ...
        if cross_knn_index is not None:
            # print(norm_v.shape, cross_knn_index.shape, norm_q.shape)
...
if __name__ == '__main__':
    model = PCTransformer(knn_layer=1).cuda()
    data = torch.randn(2, 2048, 3).cuda()
    q, coarse = model(data)
    print(q.shape, coarse.shape)
