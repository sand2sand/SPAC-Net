...
def fps(data, number, if_idx=False):
    '''
        data B N 3
        number int
    '''
    ...
    if if_idx:
        return fps_idx.squeeze().contiguous()
...
def seprate_point_cloud(xyz, num_points, crop, fixed_points=None, padding_zeros=False):
    ...
    for points in xyz:
        ...
        if isinstance(crop, list):
            fps_idx = fps(input_data, 2048, True)
            fps_idx = torch.sort(fps_idx)[0]
            input_data = input_data[:, fps_idx.type(torch.long), :]
    ...
    return input_data.contiguous(), None
...
def get_ptcloud_img(ptcloud):
    ...
    ax.scatter(x, y, z, zdir='z', c='m', cmap='jet')
